...
inline std::vector<std::string> parse(const std::string& args);
inline void stringToArgcArgv(const std::string& str, int* argc, char*** argv, bool add_NULL_at_end);
inline void freeArgv(char*** argv, int argc, bool add_NULL_at_end)
{
	if(add_NULL_at_end)
	{
		argc--;
...
inline void stringToArgcArgv(const std::string& str, int* argc, char*** argv, bool add_NULL_at_end)
{
	...
	*argv = (char**)std::malloc(*argc * sizeof(char*));
	...
	int i=0;
	for(std::vector<std::string>::iterator it = args.begin(); it != args.end(); ++it, ++i)
	{
		std::string arg = *it;
