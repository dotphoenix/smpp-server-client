...
void generate_hash_md5_bin(const unsigned char *input,	/* input block */
					   unsigned int input_len	/* length of input block */,
					   unsigned char* md5)
{
	osip_MD5_CTX Md5Ctx;
	osip_MD5Init(&Md5Ctx);
	osip_MD5Update(&Md5Ctx, (unsigned char*)input, input_len);
	osip_MD5Final(md5, &Md5Ctx);
}

void generate_hash_md5(const unsigned char *input,	/* input block */
					unsigned int input_len	/* length of input block */,
					std::string& output_hex)
{
	HASH HA1;
	HASHHEX HA1HEX;
	generate_hash_md5_bin(input, input_len, (unsigned char*)HA1);
	CvtHex(HA1, HA1HEX);
	output_hex = HA1HEX;
}
