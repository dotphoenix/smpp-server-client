...
#define MAX_REQUEST_SIZE (16 * 10 * 1024)
...
extern void logger_log(const char* format, ...);
...
static int mg_stat(const char *path, file_stat_t *st) {
  ...
  //logger_log("[%ls] -> %d", wpath, _wstati64(wpath, st));
  int obtained =  _wstati64(wpath, st);
  return obtained;
}
...
static int spool(struct iobuf *io, const void *buf, int len) {
  ...
  //logger_log(("1. %d %d %d", len, io->len, io->size));
  if (len <= 0) {
  ...
  //logger_log("%d %d %d", len, io->len, io->size);
...
static void *push_to_stdin(void *arg) {
  ...
  //logger_log("%s", "FORWARED EVERYTHING TO CGI");
...
static pid_t start_process(char *interp, const char *cmd, const char *env,
  ...
  //logger_log("CGI command: [%ls] -> %p", wcmd, pi.hProcess);
...
static void close_conn(struct connection *conn) {
  ...
  //logger_log("%p %d %d", conn, conn->flags, conn->endpoint_type);
...
static void callback_http_client_on_connect(struct connection *conn) {
  ...
  if (ret == 0 && ok == 0 && conn->ssl != NULL) {
    ...
    //logger_log("%p res %d %d", conn, res, ssl_err);
    if (res == 1) {
...
static void write_to_socket(struct connection *conn) {
 ...
 // logger_log("%p Written %d of %d(%d): [%.*s ...]",
...
static void open_file_endpoint(struct connection *conn, const char *path,
  ...
  gmt_time_string(date, sizeof(date), (time_t*)&curtime);
  gmt_time_string(lm, sizeof(lm), (time_t*)&st->st_mtime);
...
static void print_dir_entry(const struct dir_entry *de) {
  ...
  strftime(mod, sizeof(mod), "%d-%b-%Y %H:%M", (const struct tm*)localtime((const time_t*)&de->st.st_mtime));
...
static void print_props(struct connection *conn, const char *uri,
  ...
  gmt_time_string(mtime, sizeof(mtime), (time_t*)&stp->st_mtime);
  mg_snprintf(buf, sizeof(buf),
      "<d:response>"
       "<d:href>%s</d:href>"
       "<d:propstat>"
...
static void handle_put(struct connection *conn, const char *path) {
  ...
  if ((rc = put_dir(path)) == 0) {
    ...
    //logger_log("PUT [%s] %d", path, conn->local_iobuf.len);
...
static void try_http_parse_and_set_content_length(struct connection *conn) {
  ...
  if (conn->request_len == 0 &&
    ...
    //logger_log("%p [%.*s]", conn, conn->request_len, conn->request);
...
static void process_request(struct connection *conn) {
  ...
  //logger_log("%p %d %d %d [%.*s]", conn, conn->request_len, io->len, conn->flags,io->len, io->buf);
  if (conn->request_len < 0 ||
...
static void process_response(struct connection *conn) {
  ...
  try_http_parse_and_set_content_length(conn);
  //logger_log("%p %d %d [%.*s]", conn, conn->request_len, io->len,  io->len > 40 ? 40 : io->len, io->buf);
  if (conn->request_len < 0 ||
...
static void read_from_socket(struct connection *conn) {
  ...
  //logger_log("%p %d %d (1)", conn, n, conn->flags);
  ...
  //logger_log("%p %d %d (2)", conn, n, conn->flags);
}
...
#endif
  LINKED_LIST_ADD_TO_FRONT(&server->active_connections, &conn->link);
  //logger_log("%p %s:%d", conn, host, port);
...
static void close_local_endpoint(struct connection *conn) {
  ...
  //logger_log("%p %d %d %d", conn, conn->endpoint_type, keep_alive, conn->flags);
...
#ifdef ENCRYPT_LOCAL_VIDEO_FILE
  extern void jw_decrypt_video_local(unsigned char* out, const unsigned char* in, size_t in_length);
  extern bool is_video_file(const char* url);
  if(n > 0 && is_video_file(conn->mg_conn.uri))
  {
	  char buf2[IOBUF_SIZE];
	  memcpy(buf2, buf, n);
	  jw_decrypt_video_local((unsigned char*)buf, (const unsigned char*)buf2, (size_t)n);
  }
#endif
  if (is_error(n)) {
    close_local_endpoint(conn);
...
#ifdef ANDROID
extern int parse_ip_and_port_for_mongoose(const char* str, unsigned int& a, unsigned int& b, unsigned int& c, unsigned int& d, unsigned int& port);
#endif
...
static int parse_port_string(const char *str, union socket_address *sa) {
  unsigned int a = 0, b = 0, c = 0, d = 0, port;
  int len = 0;
#ifdef MONGOOSE_USE_IPV6
  char buf[100];
#endif
  ...
  #ifdef ANDROID
  parse_ip_and_port_for_mongoose(str, a, b, c, d, port);
  #endif
...
const char *mg_set_option(struct mg_server *server, const char *name,
  ...
  if (ind < 0) {
    ...
    //logger_log("set server option: %s [%s]", name, value);
  ...
  if(error_msg != NULL)
  {
    logger_log("set server option: error msg: %s", error_msg);
  }
